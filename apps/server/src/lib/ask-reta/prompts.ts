...
export const askRetaSynthesisSystemPrompt = () =>
  [
    ...
    '"cites": every entry MUST be {"ref","quote"} where ref is a source with "kind":"message" and quote is the exact words copied VERBATIM from that source excerpt. Quotes are verified server-side: altered quotes, missing quotes, or refs to "kind":"metadata" sources are discarded. Metadata sources (subject/sender rows) exist only to locate threads — never cite them. If no message source supports the answer, return "cites":[] and say the mailbox content does not confirm it.',

...
export function ThreadContextMenu({
  ...
  // #30 (suite M2): monté par chaque rang de la liste, ce menu déclenchait 15 mail.get
  // (~1,6 MB) à chaque cache client froid. enabled:false lit le cache sans fetch — le
  // préchargement au survol du rang (mail-list-thread) fournit les données au clic droit.
  const { data: threadData } = useThread(threadId, { enabled: false });

# MCP draft-loop — builder 4 Gmail threading correction

MIRROR: BUILDER

BASELINE: 81b3a66e Merge branch 'factory/niveau10' into job/niveau10/mcp-draft-loop-01
RULING: docs/jobs/niveau10/mcp-draft-loop-rulings.md
SCOPE: decisive Gmail reply-threading correction only; spec/check files unchanged; no commit or push

## PHASE 0

- Plan: validate and derive the source RFC identity in `agent-drafts`, carry a distinct
  server-only reply-header object through the draft-loop dependency, revalidate and serialize it
  in Gmail MIME, then prove the public MCP-to-Gmail path plus zero mutation on invalid identities.
- Disagreements: none. The current `ParsedMessage.messageId` and `references` fields provide the
  required server-side seam; no MCP schema, shared provider schema, spec, check, Outlook behavior,
  or wider boundary amendment is needed.

## Builder 4 correction

- `createReplyDraft` now derives the owned source message's RFC `Message-ID` before the
  idempotency reservation. Missing, malformed, control-bearing, or CRLF-injected identities
  therefore produce zero idempotency writes and zero provider draft effects.
- The provider message ID remains distinct for Outlook Graph `createReply`. Server-only
  `In-Reply-To` and normalized `References` values are carried through an internal dependency
  field that is absent from the MCP input schema and revalidated at the Gmail driver boundary.
- Gmail MIME now emits validated `In-Reply-To` and `References` in addition to the owned API
  `threadId`, derived subject, and derived recipients. No client threading header is accepted.
- The focused suite calls public `createReplyDraft` without headers, captures the Gmail request,
  decodes its base64url MIME, verifies both threading headers, `threadId`, decoded subject and
  absence of injected CRLF headers. Three rejection cases prove zero mutation.
- Outlook `createReply`, provider-normalized same-ID/fresh-revision updates, stale zero mutation,
  ownership indistinguishability, 20-way atomic idempotency, draft-only registration and zero send
  remain covered by the same suite.

## Builder 4 frozen commands
...
OUTPUT_BEGIN
 ...
 ✓ src/routes/agent/mcp-tools.test.ts (23 tests) 16ms
 ✓ src/routes/agent/mcp-draft-loop.test.ts (14 tests) 61ms

 Test Files  2 passed (2)
      Tests  37 passed (37)
   Start at  03:51:11
   Duration  871ms (transform 181ms, setup 0ms, collect 727ms, tests 77ms, environment 0ms, prepare 93ms)
...
COUNTS: frozen_commands=6 frozen_passed=6 frozen_failed=0 focused_test_files=2 focused_tests=37 focused_tests_passed=37 focused_tests_failed=0 gmail_raw_mime_cycles=1 invalid_source_identity_cases=3 provider_normalization_cycles=1 provider_seam_tests=5 security_surface_failures=0 eslint_errors=0 typecheck_errors=0 touch_set_violations=0 diff_check_errors=0
STATUS: COMPLETE

## Retained builder 3 evidence (superseded by builder 4 above)

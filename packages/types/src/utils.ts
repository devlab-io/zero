// Shared mail utilities (pitbull quality/pitbull, GAP 1). Moved from
// apps/mail/lib/utils.ts + apps/server/src/lib/utils.ts, which had drifted
// into ~180 lines of duplicated logic — including a real divergence in
// constructReplyBody (server supports an optional quoted-message block, the
// client copy had silently dropped it). The server version is authoritative
// here (see packages/types/src/email-utils.ts for the same rule applied to
// address parsing). Pure functions + constants only, no DOM/env access, so
// this stays safe to bundle on the client.
import type { Sender } from './email-utils';
...
// NOTE (GAP 2/quality-pitbull): the client copy of this function had dropped the
// `quotedMessage` parameter entirely (its only call site, reply-composer.tsx,
// even has the argument commented out). The server version — which is what
// actually executes — supports it, so it wins here per "server fait foi". This
// means client-built reply bodies will now include an (empty, since nothing
// currently passes quotedMessage) `<div>` wrapper they didn't emit before. It's
// content-less and shouldn't render visibly, but flagging it since it's a
// visible-in-source change to sent email HTML that was previously silent.
...
// Server has no equivalent of constructForwardBody (only the client composes
// forwards), so there is no cross-side divergence to reconcile here — this is
// the client's implementation, moved as-is.
export const constructForwardBody = (
  formattedMessage: string,
  originalDate: string,
  originalSender: (Sender & { subject?: string }) | undefined,
  otherRecipients: Sender[],
) => {
  const senderName = originalSender?.name || originalSender?.email || 'Unknown Sender';
  const recipientEmails = otherRecipients.map((r) => r.email).join(', ');

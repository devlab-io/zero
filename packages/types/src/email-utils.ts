// Shared email-parsing utilities (pitbull quality/pitbull, GAP 1). Moved from
// apps/mail/lib/email-utils.ts + apps/server/src/lib/email-utils.ts, which had
// drifted into ~150 lines of near-identical duplicated logic. Pure functions +
// constants only — no DOM, no server-only imports, so this is safe to bundle
// on the client.
...
//
// `onParseError` is an optional side-effect hook (e.g. server-side debug logging)
// so this stays a pure function with no logger dependency baked into the shared
// package — the server wrapper in apps/server/src/lib/email-utils.ts supplies it.
